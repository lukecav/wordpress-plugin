...
class MailgunAdmin extends Mailgun
{
    ...
    public function admin_notices()
    {
        ...
        if ((empty($this->options['apiKey']) && $this->options['useAPI'] == '1')
            || (empty($this->options['password']) && $this->options['useAPI'] == '0')
        ) { ?>
...
<?php
        ...
        if ($this->options['override-from'] == '1'
            && (empty($this->options['from-name'])
            || empty($this->options['from-address']))
        ) { ?>
